...
export function createFilterApplier<T>(
  filterFunctionMapping: FilterFunctionMapping<T>
) {
  return (values: T[], filters: Filters): T[] => {
    return values.filter((values) => {
      return Object.entries(filters).every(([filterType, filterValues]) => {
        // If no filters are applied for the specific type we need to handle the case separately
        if (filterValues.length === 0) {
          return true;
        }

        const filterFunc =
          filterFunctionMapping[filterType as any as FilterType]; // TODO: Fix typings
        return (filterValues || []).some((filterValue) =>
          filterFunc(values, filterValue)
        ); //filterFunctionMapping
      });
    });
  };
}
